...
ClassImp(readtruth_hhbbttlvlv)

readtruth_hhbbttlvlv :: readtruth_hhbbttlvlv ()
{
...
TH1F* readtruth_hhbbttlvlv :: mkTH1F( const char* n, const char* t, double nb, double x0, double x1){
...
void readtruth_hhbbttlvlv :: setOption ( std::string opt ){
  option = opt;
}

TH2F* readtruth_hhbbttlvlv::mkTH2F( const char* n, const char* t, double xnb, double x0, double x1, double ynb, double y0, double y1 ){
...
const xAOD::TruthParticle* readtruth_hhbbttlvlv :: correctedParticle( const xAOD::TruthParticle * part ){
...
// does not work ...
const xAOD::TruthParticle* readtruth_hhbbttlvlv :: correctedQuark( const xAOD::TruthParticle * part ){
  //Info("correctedParticle()", "Check particle with barcode %d, status %d", part->barcode(), part->status() );
  const xAOD::TruthParticle* corrpart = part;
  for( unsigned ikid=0; ikid<part->nChildren(); ++ikid ){
    const xAOD::TruthParticle* kid = part->child( ikid );
...
bool readtruth_hhbbttlvlv :: hasParent( const xAOD::TruthParticle* kid, int parentId ){
...
void readtruth_hhbbttlvlv :: printChildren( const xAOD::TruthParticle* part ){
...
// not working at all !
TLorentzVector readtruth_hhbbttlvlv :: getJet( const xAOD::TruthParticle* part ){
  TLorentzVector jet;
  for( unsigned int ikid=0; ikid<part->nChildren(); ++ikid ){
...
bool readtruth_hhbbttlvlv :: larger_pT( const xAOD::TruthParticle* p1, const xAOD::TruthParticle* p2 ){
  return p1->pt() > p2->pt();
}

EL::StatusCode readtruth_hhbbttlvlv :: setupJob (EL::Job& job)
{
...
EL::StatusCode readtruth_hhbbttlvlv :: histInitialize ()
{
  ...
  // for bbll tt these will be empty !!!
  ...
  // register
  //for_each( myhist.begin(), myhist.end(), std::bind1st( std::mem_fun( &readtruth_hhbbttlvlv::mbook ), this ) );
  // now done in mkTH1F
...
EL::StatusCode readtruth_hhbbttlvlv :: fileExecute ()
{
...
EL::StatusCode readtruth_hhbbttlvlv :: changeInput (bool firstFile)
{
...
EL::StatusCode readtruth_hhbbttlvlv :: initialize ()
{
...
EL::StatusCode readtruth_hhbbttlvlv :: execute ()
{
...
//  }
  ...
  std::vector<const xAOD::TruthParticle*> myleptons; // all e mu leptons ordere by pT from e mu and even tau
  ...
  for( int ipart=0; ipart<npart; ++ipart ){
	...
	if( abs(particle->pdgId()) == 15 ){
	  ...
	  if( mycollection.count("tau1") == 0 ) mycollection["tau1"] = particle;
	  else{
	    if( particle != mycollection["tau1"] ) mycollection["tau2"] = particle;
	  }
	}

	// once found H1 H2 quit loop (only used without looking into H decays)
	if( mycollection.count("H1")==1 && mycollection.count("H2")==1
	&&  mycollection.count("tau1")==1 && mycollection.count("tau2")==1 )
	  break;
  ...
  _h1_pT = mycollection["tau1"]->pt();
  _h2_pT = mycollection["tau2"]->pt();
  if( _h1_pT < _h2_pT ){
    const xAOD::TruthParticle* _h2 = mycollection["tau1"];
    const xAOD::TruthParticle* _h1 = mycollection["tau2"];
	mycollection["tau1"] = _h1;
	mycollection["tau2"] = _h2;
  }
  ...
  // find lvlv from tautau
  for( unsigned int ikid=0; ikid<mycollection["tau1"]->nChildren(); ++ikid){
    const xAOD::TruthParticle* kid = mycollection["tau1"]->child(ikid);
	if( abs(kid->pdgId()) == 11){
	  kid = correctedParticle( kid ); // after shower dressing
	  ...
	  if( std::find(mytaus.begin(), mytaus.end(), kid) == mytaus.end() ){
		...
		// look into lep tau decays for minic truth filter in JO
	  ...
	  // bbtautau lvlv different than bbZZ, v here has all flavours
	  kid = correctedParticle( kid ); // after shower dressing
  ...
  for( unsigned int ikid=0; ikid<mycollection["tau2"]->nChildren(); ++ikid){
    const xAOD::TruthParticle* kid = mycollection["tau2"]->child(ikid);
	if( abs(kid->pdgId()) == 11){
	  kid = correctedParticle( kid ); // after shower dressing
	  if( std::find(myelectrons.begin(), myelectrons.end(), kid) == myelectrons.end() ){
	    myelectrons.push_back(kid);
  ...
  if( myneutrinos.size() < 2 || myleptons.size() < 2 ){
	...
	Error("execute()","SKIP this buggy event!!!");
	return EL::StatusCode::SUCCESS;
  }
...
EL::StatusCode readtruth_hhbbttlvlv :: postExecute ()
{
...
EL::StatusCode readtruth_hhbbttlvlv :: finalize ()
{
  ...
  Error("finalize()", "Total events: %d, truth identified (HH+tautau) events: %d", ctr_tot, int(ctr_save) );

  Error("finalize()", "tau -> enu events: %f", el1_pT->GetEntries() );
  Error("finalize()", "tau -> munu events: %f", mu1_pT->GetEntries() );
  Error("finalize()", "tau -> taunu events: %f", tau1_pT->GetEntries() );
...
EL::StatusCode readtruth_hhbbttlvlv :: histFinalize ()
{
