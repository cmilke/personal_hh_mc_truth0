...
EL::StatusCode readtruth_hhbbzzllvv :: histInitialize ()
{
  ...
  // lepton
  el1_pT = mkTH1F("el1_pT","p_{T}(electron 1)", 50, 0, 800);
  el1_eta = mkTH1F("el1_eta","#eta(electron 1)", 40, -5, 5);
  el1_phi = mkTH1F("el1_phi","#phi(electron 1)", 40, -3.15, 3.15 );
  el1_E = mkTH1F("el1_E","E(electron 1)", 50, 0, 2000);

  el2_pT = mkTH1F("el2_pT","p_{T}(electron 2)", 50, 0, 800);
  el2_eta = mkTH1F("el2_eta","#eta(electron 2)", 40, -5, 5);
  el2_phi = mkTH1F("el2_phi","#phi(electron 2)", 40, -3.15, 3.15 );
  el2_E = mkTH1F("el2_E","E(electron 2)", 50, 0, 2000);

  mu1_pT = mkTH1F("mu1_pT","p_{T}(muon 1)", 50, 0, 800);
  mu1_eta = mkTH1F("mu1_eta","#eta(muon 1)", 40, -5, 5);
  mu1_phi = mkTH1F("mu1_phi","#phi(muon 1)", 40, -3.15, 3.15 );
  mu1_E = mkTH1F("mu1_E","E(muon 1)", 50, 0, 2000);

  mu2_pT = mkTH1F("mu2_pT","p_{T}(muon 2)", 50, 0, 800);
  mu2_eta = mkTH1F("mu2_eta","#eta(muon 2)", 40, -5, 5);
  mu2_phi = mkTH1F("mu2_phi","#phi(muon 2)", 40, -3.15, 3.15 );
  mu2_E = mkTH1F("mu2_E","E(muon 2)", 50, 0, 2000);

  tau1_pT = mkTH1F("tau1_pT","p_{T}(tau 1)", 50, 0, 800);
  tau1_eta = mkTH1F("tau1_eta","#eta(tau 1)", 40, -5, 5);
  tau1_phi = mkTH1F("tau1_phi","#phi(tau 1)", 40, -3.15, 3.15 );
  tau1_E = mkTH1F("tau1_E","E(tau 1)", 50, 0, 2000);

  tau2_pT = mkTH1F("tau2_pT","p_{T}(tau 2)", 50, 0, 800);
  tau2_eta = mkTH1F("tau2_eta","#eta(tau 2)", 40, -5, 5);
...
//  }
  ...
  for( unsigned int ikid=0; ikid<mycollection["Z1"]->nChildren(); ++ikid){
	...
	}else if( abs(kid->pdgId()) == 12 ){ // in bbZZ llvv v is only 12
	  kid = correctedParticle( kid ); // after shower dressing
